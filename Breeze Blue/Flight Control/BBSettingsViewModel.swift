...
protocol BBSettingsViewModelDelegate: class {
    func didUpdate()
}

protocol BBSettingsViewModelObserver: BBObserver {
    func didUpdate(controlMode: ControlMode)
}

final class BBSettingsViewModel: BBObservable {
    typealias ConcreteObserver = BBSettingsViewModelObserver

    weak var delegate: BBSettingsViewModelDelegate?
...
//    private let bluetoothLink: BluetoothLink
    ...
    init(sensitivity: Sensitivity, controlMode: ControlMode) {
//        self.bluetoothLink = bluetoothLink
    ...
    var hasTiltMode: Bool {
        return BBMotionLink().canAccessMotion
    }
